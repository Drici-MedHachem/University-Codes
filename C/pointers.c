...
int main(void) {
	...
	char (*ptr)[5]; // pointer to an array of 5 chars
	
	int mat[n][m];
	mat[i]; // pointer to the ith row 
	int (* mat_ptr)[n] = mat; // can use mat_ptr[i][j] normally
	int ** mat_p = &mat[0][0]; // makes the matrix flat and thus you can access
							   // from 0 to n*m-1
